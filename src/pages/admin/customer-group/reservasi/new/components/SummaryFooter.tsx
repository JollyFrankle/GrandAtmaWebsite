...
import { KamarDipesan, SummaryKamarDipesan } from "../PageRoomSearchCG"
...
export default function SummaryFooter({
    ...
    return (
        <footer className={`sticky left-0 bottom-2 right-0 transition-transform duration-500 ease-in-out transform`}>
            <div className="p-4 rounded-xl bg-background shadow-lg mb-4">
                <div className="md:flex justify-between items-center">
                    <div className="hidden md:block">
                        <div className="text-sm text-muted-foreground">Kamar yang dipesan:</div>
                        <div className="text-lg font-bold">{kamarDipesan.length > 0 ? kamarDipesan.map((item) => `${item.count} ${item.nama}`).join(", ") : 'Belum ada'}</div>
                    </div>
